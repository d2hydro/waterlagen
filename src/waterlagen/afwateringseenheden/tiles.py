...
def calculate_afwateringseenheden_tiles(
    ...
    *,
    ...
    tile_results: list[AfwateringseenhedenTileResult] = []
    boundary_issue_tile_ids: list[str] = []
    skipped_tile_ids: list[str] = []

    for tile in selected_tiles:
        tile_output_dir = output_dir / tile.tile_id
        ...
        if existing_subcatchments is None:
            rasters = prepare_watersysteem_rasters(
                _calculation_geometry(tile, tile_buffer_m=tile_buffer_m),
            ...
            if not _has_segment_cells(rasters):
                ...
                skipped_tile_ids.append(tile.tile_id)
                tile_results.append(
        ...
        if has_boundary_issue:
            boundary_issue_tile_ids.append(tile.tile_id)
            logger.warning(
